...
def convert_png_to_svg(png_path: Path, svg_path: Path):
    try:
        ...
        svg_content = f'<?xml version="1.0" encoding="UTF-8"?><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="{width}" height="{height}" viewBox="0 0 {width} {height}"><image width="{width}" height="{height}" xlink:href="data:image/png;base64,{b64}"/></svg>'
...
async def health_check():
    ...
    return {"status": "ok", "api_key_configured": has_key, "api_key_preview": f"{api_key[:8]}..." if has_key else "NOT SET"}
...
async def generate_figure(request: GenerateRequest):
    ...
    try:
        ...
        if process.returncode != 0:
            ...
            raise HTTPException(status_code=500, detail=f"PaperBanana failed: {error_msg[:500]}")
        png_file = None
        ...
        return GenerateResponse(job_id=job_id, status="success", png_url=f"/outputs/{job_id}/figure.png", svg_url=f"/outputs/{job_id}/figure.svg", message="Figure generated successfully!")
...
ENDOFFILEcat > ~/Desktop/paperbanana-web/app.py << 'ENDOFFILE'
import os
import uuid
import asyncio
import shutil
